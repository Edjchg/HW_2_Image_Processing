%-*- texinfo -*-
%@deftypefn {Function File} {@var{imagen_limpia} =}  impr_med(@var{I})
%Function impr_med()
%@end deftypefn

function imagen_limpia = impr_med(I)
    ...
    for i=1:m
      ...
      for j=1:n
        ...
        if i_left && i_right && j_left && j_right %Revisando cuando no esta en los bordes que es la mayoria del tiempo.
          C=mid_value_decision_median(I(i-1:i+1,j+1,z),3); %Tomando el nuevo p?xel de la columna C.
        ...
        elseif i_init && j_init %Revisando la esquina superior izquierda.
          C=mid_value_decision_median(I(i:i+1,j+1,z),2); %Tomando el nuevo p?xel de la columna C.
